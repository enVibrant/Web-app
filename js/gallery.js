...
.then(response => 
{
  ...
  for( var i = 0; i < response.image.length; i++)
  {
    ...
    imgtag.src = response.image[i].imageUrl; 
     var url  = `https://venturesketch-website.venturesketch.repl.co/html/artGallery2.html?artistName=${response.image[i].artistName}&imageName=${response.image[i].imageUrl}`;
   ...
   var rightDiv = document.createElement("div");
    ...
    cButton.addEventListener('click', function () {
            window.open("/html/galleryComment.html");
